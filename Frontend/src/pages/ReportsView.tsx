    ...
    return (
      <div className="app-page-shell rf-page rf-page--reports space-y-5">
        ...
        <section className="rf-data-panel rounded-2xl border border-[var(--border-light)] bg-[var(--bg-surface)] shadow-sm">
          <div className="flex flex-col gap-3 border-b border-[var(--border-light)] px-5 py-4 lg:flex-row lg:items-end lg:justify-between"><div><h2 className="font-extrabold text-[var(--text-primary)]">Generated report history</h2><p className="mt-1 text-xs text-[var(--text-muted)]">Generated PowerPoint files stay here until you explicitly download or delete them.</p></div><div className="flex flex-wrap gap-2"><input aria-label="Search generated reports" value={historyFilters.search || ''} onChange={(event) => updateHistoryFilter('search', event.target.value)} placeholder="Search history" className="min-h-10 rounded-xl border border-[var(--input-border)] bg-[var(--input-bg)] px-3 text-sm text-[var(--text-primary)] outline-none focus:border-[var(--sgh-cyan-primary)] focus:ring-2 focus:ring-[var(--sgh-cyan-primary)]/20 transition-all" /><select aria-label="History report type" value={historyFilters.report_type || ''} onChange={(event) => updateHistoryFilter('report_type', event.target.value)} className="min-h-10 rounded-xl border border-[var(--input-border)] bg-[var(--input-bg)] px-3 text-sm text-[var(--text-primary)] outline-none focus:border-[var(--sgh-cyan-primary)] focus:ring-2 focus:ring-[var(--sgh-cyan-primary)]/20 transition-all"><option value="">All report types</option>{Array.from(new Set(templates.map((template) => template.type))).map((type) => <option key={type} value={type}>{type}</option>)}</select><select aria-label="History period" value={historyFilters.period || ''} onChange={(event) => updateHistoryFilter('period', event.target.value)} className="min-h-10 rounded-xl border border-[var(--input-border)] bg-[var(--input-bg)] px-3 text-sm text-[var(--text-primary)] outline-none focus:border-[var(--sgh-cyan-primary)] focus:ring-2 focus:ring-[var(--sgh-cyan-primary)]/20 transition-all"><option value="">All periods</option>{options.periods.map((period) => <option key={period.key} value={`${period.month} ${period.year}`}>{period.month} {period.year}</option>)}</select><select aria-label="History status" value={historyFilters.status || ''} onChange={(event) => updateHistoryFilter('status', event.target.value)} className="min-h-10 rounded-xl border border-[var(--input-border)] bg-[var(--input-bg)] px-3 text-sm text-[var(--text-primary)] outline-none focus:border-[var(--sgh-cyan-primary)] focus:ring-2 focus:ring-[var(--sgh-cyan-primary)]/20 transition-all"><option value="">All statuses</option><option value="ready">Ready</option><option value="failed">Failed</option></select><button type="button" onClick={() => reportsQuery.refetch()} aria-label="Refresh reports" className="min-h-10 min-w-10 rounded-xl border border-[var(--border-light)] text-[var(--text-muted)] hover:bg-[var(--bg-sunken)] hover:text-[var(--sgh-cyan-primary)] transition-colors"><RefreshCw size={16} className={`mx-auto ${reportsQuery.isFetching ? 'animate-spin' : ''}`} /></button></div></div>
          ...
          <div className="overflow-x-auto"><table className="min-w-[900px] w-full text-left text-sm"><thead className="bg-[var(--bg-sunken)]/70 text-[10px] font-extrabold uppercase tracking-wider text-[var(--text-faint)]"><tr><th className="w-12 px-5 py-3">{canExport && <input type="checkbox" aria-label="Select all reports on this page" checked={allSelected} onChange={toggleAll} disabled={!reports.length} className="h-4 w-4 rounded border-[var(--border-medium)] accent-[#00A3E0]" />}</th><th className="px-5 py-3">Report name</th><th className="px-4 py-3">Type</th><th className="px-4 py-3">Period</th><th className="px-4 py-3">Created at</th><th className="px-4 py-3">Format</th><th className="px-5 py-3 text-right">Actions</th></tr></thead><tbody>{reports.map((report) => { const meta = formatMeta(report.format); const FormatIcon = meta.icon; return <tr key={report.id} className="border-t border-[var(--border-light)] text-[var(--text-secondary)]"><td className="px-5 py-4">{canExport && <input type="checkbox" aria-label={`Select ${report.name}`} checked={selectedReportIds.includes(report.id)} onChange={() => toggleSelection(report.id)} className="h-4 w-4 rounded border-[var(--border-medium)] accent-[#00A3E0]" />}</td><td className="px-5 py-4 font-bold text-[var(--text-primary)]">{report.name}</td><td className="px-4 py-4">{report.report_type}</td><td className="px-4 py-4 whitespace-nowrap">{report.period}</td><td className="px-4 py-4 whitespace-nowrap">{formatDate(report.created_at)}</td><td className="px-4 py-4"><span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-2 py-1 text-xs font-bold text-emerald-700 dark:text-emerald-300"><FormatIcon size={13} /> {meta.label}</span></td><td className="px-5 py-4"><div className="flex justify-end gap-2">{canExport && <button type="button" aria-label={`Download ${report.name}`} onClick={() => downloadReport(report)} disabled={downloadingId === report.id} className="flex min-h-9 min-w-9 items-center justify-center rounded-lg border border-[var(--border-light)] text-[var(--sgh-cyan-primary)] hover:bg-[var(--bg-sunken)] hover:border-[var(--sgh-cyan-primary)]/30 transition-colors disabled:opacity-50">{downloadingId === report.id ? <Loader2 size={15} className="animate-spin" /> : <Download size={15} />}</button>}{canExport && <button type="button" aria-label={`Delete ${report.name}`} onClick={() => setDeleteCandidate(report)} className="flex min-h-9 min-w-9 items-center justify-center rounded-lg border border-rose-500/20 text-rose-600 hover:bg-rose-500/10 hover:border-rose-500/40 transition-colors"><Trash2 size={15} /></button>}</div></td></tr>; })}{!reportsQuery.isLoading && reports.length === 0 && <tr><td colSpan={7} className="px-5 py-12 text-center text-sm text-[var(--text-muted)]">No generated reports yet.</td></tr>}</tbody></table></div>
          {selectedReports.length > 0 && canExport && <div className="flex justify-end border-t border-[var(--border-light)] px-5 py-3"><button type="button" onClick={() => setDeleteCandidates(selectedReports)} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-rose-500/20 px-3 text-xs font-extrabold text-rose-600 hover:bg-rose-500/10 hover:border-rose-500/40 transition-colors"><Trash2 size={15} /> Delete {selectedReports.length} selected report{selectedReports.length === 1 ? '' : 's'}</button></div>}
          {(reportsQuery.data?.total || 0) > 10 && <div className="flex items-center justify-end gap-2 border-t border-[var(--border-light)] px-5 py-3"><button disabled={page === 1} onClick={() => { setSelectedReportIds([]); setPage((value) => Math.max(1, value - 1)); }} className="rounded-lg border border-[var(--border-light)] px-3 py-2 text-xs font-bold text-[var(--text-secondary)] hover:bg-[var(--bg-sunken)] hover:text-[var(--text-primary)] transition-colors disabled:opacity-40">Previous</button><span className="text-xs text-[var(--text-muted)]">Page {page}</span><button disabled={page * 10 >= (reportsQuery.data?.total || 0)} onClick={() => { setSelectedReportIds([]); setPage((value) => value + 1); }} className="rounded-lg border border-[var(--border-light)] px-3 py-2 text-xs font-bold text-[var(--text-secondary)] hover:bg-[var(--bg-sunken)] hover:text-[var(--text-primary)] transition-colors disabled:opacity-40">Next</button></div>}
        </section>
