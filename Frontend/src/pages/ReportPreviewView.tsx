...
export default function ReportPreviewView() {
  ...
  return (
    <div className="app-page-shell rf-page rf-page--report-preview space-y-5">
      ...
      {draft.validation && <section className={`rounded-2xl border p-4 ${draft.validation.valid ? 'border-emerald-500/20 bg-emerald-500/5' : 'border-amber-500/20 bg-amber-500/5'}`}><div className="flex items-center gap-2 font-extrabold text-[var(--text-primary)]">{draft.validation.valid ? <CheckCircle2 size={18} className="text-emerald-600 dark:text-emerald-400" /> : <AlertCircle size={18} className="text-amber-600 dark:text-amber-400" />}{draft.validation.valid ? 'This draft is ready for review.' : 'This draft still has review warnings.'}</div>{draft.validation.issues.length > 0 && <ul className="mt-3 space-y-1 text-xs font-semibold text-[var(--text-secondary)]">{draft.validation.issues.map((issue, index) => <li key={`${issue.code}-${index}`}>{issue.severity.toUpperCase()}: {issue.message}</li>)}</ul>}</section>}

      {!activePage ? <div className="grid min-h-[45vh] place-items-center rounded-2xl border-2 border-dashed border-[var(--border-light)] bg-[var(--bg-surface)] text-sm font-semibold text-[var(--text-muted)]">This draft has no pages yet.</div> : <section aria-label={`Preview page ${pageIndex + 1}`} className="rf-preview-canvas rounded-3xl border border-[var(--border-light)] bg-[var(--bg-surface)] p-4 shadow-xl md:p-8"><div className="mb-5 flex items-center justify-between gap-3 border-b border-[var(--border-light)] pb-4"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.16em] text-[var(--sgh-cyan-primary)]">Page {pageIndex + 1} of {pages.length}</p><h2 className="mt-1 text-2xl font-black text-[var(--text-primary)]">{activePage.title}</h2></div>{isFetching && <span className="inline-flex items-center gap-2 text-xs font-bold text-[var(--text-muted)]"><Loader2 size={14} className="animate-spin" /> Refreshing evidence</span>}</div>{pageLoading ? <PanelLoadingSkeleton rows={4} label={`Loading ${activePage.title}`} /> : <div className={`grid gap-5 ${activePage.blocks.length > 1 ? 'md:grid-cols-2' : 'grid-cols-1'}`}>{activePage.blocks.map((block) => <div key={block.id} className="min-w-0"><BlockRenderer block={block} blockData={pageData?.blocks[block.id]} /></div>)}</div>}</section>}

      <nav aria-label="Report preview pages" className="flex items-center justify-center gap-4"><button type="button" aria-label="Previous preview page" disabled={pageIndex === 0} onClick={() => setPageIndex((value) => Math.max(0, value - 1))} className="grid min-h-11 min-w-11 place-items-center rounded-xl border border-[var(--border-light)] text-[var(--text-secondary)] hover:bg-[var(--bg-sunken)] hover:text-[var(--text-primary)] transition-colors disabled:opacity-40"><ChevronLeft size={18} /></button><span className="text-sm font-extrabold text-[var(--text-secondary)]">Page {pages.length ? pageIndex + 1 : 0} of {pages.length}</span><button type="button" aria-label="Next preview page" disabled={pageIndex >= pages.length - 1} onClick={() => setPageIndex((value) => Math.min(Math.max(pages.length - 1, 0), value + 1))} className="grid min-h-11 min-w-11 place-items-center rounded-xl border border-[var(--border-light)] text-[var(--text-secondary)] hover:bg-[var(--bg-sunken)] hover:text-[var(--text-primary)] transition-colors disabled:opacity-40"><ChevronRight size={18} /></button></nav>
    </div>
  );
}
