...
const input = 'min-h-11 rounded-xl border border-[var(--input-border)] bg-[var(--input-bg)] px-3 text-sm font-semibold text-[var(--input-text)] outline-none focus:border-[var(--sgh-cyan-primary)] focus:ring-2 focus:ring-[var(--sgh-cyan-primary)]/20 transition-all';
...
function PlanCardView({ plan, active, onClick }: { plan: PlanCard; active: boolean; onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`relative w-full overflow-hidden rounded-2xl border p-4 text-left transition ${active ? 'border-[var(--sgh-cyan-primary)] bg-[var(--sgh-cyan-primary)]/10 shadow-sm ring-1 ring-[var(--sgh-cyan-primary)]/20' : 'border-[var(--border-light)] bg-[var(--bg-surface)] hover:border-[var(--sgh-cyan-primary)]/40 hover:shadow-sm'}`}
    >
      {active && <span className="absolute inset-y-0 left-0 w-1 bg-[var(--sgh-cyan-primary)]" />}
      ...
      <div className="mt-4">
        ...
        <div className="mt-1.5 h-2 overflow-hidden rounded-full bg-[var(--bg-sunken)]"><div className="h-full rounded-full bg-gradient-to-r from-[#00A3E0] to-[#00A859]" style={{ width: `${Math.min(100, Math.max(0, plan.progress.overall))}%` }} /></div>
      </div>
  ...
  return (
    <div className="space-y-4">
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4" aria-label="Plan outcome summary">
        ...
        <SummaryMetric label="Target" value={formatMetric(detail.summary.target, detail.summary.unit)} hint={detail.summary.direction.replace('_', ' ')} icon={Target} tone="bg-[var(--sgh-cyan-primary)]/10 text-[var(--sgh-cyan-primary)]" />
        <SummaryMetric label="Current" value={formatMetric(detail.summary.current, detail.summary.unit)} hint="Latest measured result" icon={CheckCircle2} tone="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" />
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(280px,0.8fr)_minmax(340px,1.2fr)]">
        <div className="space-y-4">
          <article className="rounded-2xl border border-[var(--border-light)] bg-[var(--bg-surface)] p-4">
            ...
            ].map(([label, count, hint]) => <div key={String(label)} className={`rounded-xl border p-3 ${Number(count) > 0 ? 'border-emerald-500/20 bg-emerald-500/[0.055]' : 'border-amber-500/20 bg-amber-500/[0.055]'}`}><div className="flex items-center gap-2">{Number(count) > 0 ? <CheckCircle2 size={15} className="text-emerald-600 dark:text-emerald-400" /> : <AlertCircle size={15} className="text-amber-600 dark:text-amber-400" />}<strong className="text-xs text-[var(--text-primary)]">{label}: {count}</strong></div><p className="mt-1 text-[10px] text-[var(--text-muted)]">{hint}</p></div>)}</div>
          ...
          <article className="rounded-2xl border border-[var(--border-light)] bg-[var(--bg-surface)] p-4">
            ...
            <div className="mt-3"><div className="flex justify-between text-xs"><span className="font-bold text-[var(--text-secondary)]">Overall progress</span><strong>{detail.progress.overall}%</strong></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-[var(--bg-sunken)]"><div className="h-full rounded-full bg-gradient-to-r from-[#00A3E0] to-[#00A859]" style={{ width: `${Math.min(100, Math.max(0, detail.progress.overall))}%` }} /></div><p title={detail.progress.explanation} className="mt-2 text-[10px] leading-4 text-[var(--text-muted)]">{detail.progress.explanation}</p></div>
          </article>
        </div>

        <article className="overflow-hidden rounded-2xl border border-[var(--border-light)] bg-[var(--bg-surface)]">
          <header className="flex items-center justify-between border-b border-[var(--border-light)] px-4 py-3"><div><h3 className="font-extrabold text-[var(--text-primary)]">Linked evidence</h3><p className="mt-0.5 text-xs text-[var(--text-muted)]">Insights that justified creating this plan.</p></div><span className="rounded-full bg-[var(--sgh-cyan-primary)]/10 px-2.5 py-1 text-[10px] font-bold text-[var(--sgh-cyan-primary)]">{detail.linked_insights.length} linked</span></header>
          {detail.linked_insights.length ? <div className="custom-scrollbar max-h-[470px] space-y-2 overflow-y-auto p-3">{detail.linked_insights.slice(0, 5).map((insight) => <a key={insight.id} href="/insights" className={`block rounded-xl border p-3 transition hover:border-[var(--sgh-cyan-primary)]/40 ${insightStyle[insight.severity || 'information'] || insightStyle.information}`}><div className="flex items-start gap-3"><span className="mt-0.5 grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[var(--bg-surface)] text-[var(--sgh-cyan-primary)]"><Link2 size={14} /></span><span className="min-w-0"><strong className="block text-sm leading-5 text-[var(--text-primary)]">{insight.title || 'Insight evidence unavailable'}</strong><span className="mt-1 line-clamp-3 block text-xs leading-5 text-[var(--text-muted)]">{insight.explanation || `Evidence reference: ${insight.id}`}</span>{insight.scope && <span className="mt-2 block text-[10px] font-bold text-[var(--text-faint)]">{insight.scope}</span>}</span></div></a>)}{detail.linked_insights.length > 5 && <p className="py-2 text-center text-xs font-semibold text-[var(--text-muted)]">Showing 5 of {detail.linked_insights.length}. Open Insights to review all evidence.</p>}</div> : <EmptyState icon={Link2} title="No linked evidence" copy="This plan was created without a linked insight. Review the documented business reason before activation." />}
        </article>
      </section>

      <article className="overflow-hidden rounded-2xl border border-[var(--border-light)] bg-[var(--bg-surface)]"><header className="flex items-center justify-between border-b border-[var(--border-light)] px-4 py-3"><div><h3 className="font-extrabold text-[var(--text-primary)]">Key KPI tracking</h3><p className="mt-0.5 text-xs text-[var(--text-muted)]">Baseline-to-target measurement for this plan.</p></div>{detail.kpis.length > 5 && <button type="button" onClick={onOpenKpis} className="text-xs font-bold text-[var(--sgh-cyan-primary)] hover:underline">View all KPIs</button>}</header><KpiTable detail={detail} limit={5} /></article>

      <article className="overflow-hidden rounded-2xl border border-[var(--border-light)] bg-[var(--bg-surface)]"><header className="border-b border-[var(--border-light)] px-4 py-3"><h3 className="font-extrabold text-[var(--text-primary)]">Review milestones</h3><p className="mt-0.5 text-xs text-[var(--text-muted)]">Scheduled checkpoints to confirm that execution is producing movement.</p></header>{detail.milestones.length ? <div className="grid gap-3 p-4 md:grid-cols-2 xl:grid-cols-4">{detail.milestones.slice(0, 4).map((milestone) => <div key={milestone.id} className="rounded-xl border border-[var(--border-light)] bg-[var(--bg-sunken)]/35 p-3"><div className="flex items-center justify-between"><span className="grid h-8 w-8 place-items-center rounded-lg bg-[var(--sgh-cyan-primary)]/10 text-[var(--sgh-cyan-primary)]"><Flag size={14} /></span><span className="text-[9px] font-bold uppercase text-[var(--text-faint)]">{milestone.status}</span></div><strong className="mt-3 block text-xs text-[var(--text-primary)]">{milestone.name}</strong><p className="mt-1 text-[10px] text-[var(--text-muted)]">{formatDate(milestone.due_date)} · {milestone.owner}</p></div>)}</div> : <EmptyState icon={Flag} title="No review milestones" copy="Add at least one checkpoint so progress can be reviewed before the final due date." />}</article>
    </div>
  );
}
...
function EditPlanModal({ detail, owners, statuses, onClose }: { detail: PlanDetail; owners: Array<{ id: string; name: string }>; statuses: string[]; onClose: () => void }) {
  ...
  return <OverlayPortal><div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-3 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><form role="dialog" aria-modal="true" aria-labelledby="edit-plan-title" onSubmit={submit} className="custom-scrollbar max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-3xl border border-[var(--border-light)] bg-[var(--bg-surface)] shadow-2xl">
    ...
    <footer className="flex justify-end gap-2 border-t border-[var(--border-light)] p-4"><button type="button" onClick={onClose} className="min-h-11 rounded-xl border border-[var(--border-light)] px-4 font-bold text-[var(--text-secondary)] hover:bg-[var(--bg-sunken)] hover:text-[var(--text-primary)] transition-colors">Cancel</button><button type="submit" disabled={update.isPending} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[var(--sgh-cyan-primary)] px-5 font-bold text-white shadow-sm hover:brightness-110 active:scale-[0.98] transition-all disabled:opacity-60">{update.isPending ? <Loader2 size={16} className="animate-spin" /> : <Pencil size={16} />}Save changes</button></footer>
...
function DeletePlanModal({ detail, onClose, onDeleted }: { detail: PlanDetail; onClose: () => void; onDeleted: () => void }) {
  ...
  return <OverlayPortal><div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-3 backdrop-blur-sm"><div role="alertdialog" aria-modal="true" aria-labelledby="delete-plan-title" aria-describedby="delete-plan-description" className="w-full max-w-lg rounded-3xl border border-rose-500/20 bg-[var(--bg-surface)] p-5 shadow-2xl">
    ...
    <div className="mt-5 flex justify-end gap-2"><button type="button" onClick={onClose} className="min-h-11 rounded-xl border border-[var(--border-light)] px-4 font-bold text-[var(--text-secondary)] hover:bg-[var(--bg-sunken)] hover:text-[var(--text-primary)] transition-colors">Cancel</button><button type="button" disabled={confirmation !== detail.name || remove.isPending} onClick={submit} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-rose-600 px-5 font-bold text-white shadow-sm hover:bg-rose-500 active:scale-[0.98] transition-all disabled:cursor-not-allowed disabled:opacity-40">{remove.isPending ? <Loader2 size={16} className="animate-spin" /> : <Trash2 size={16} />}Delete plan</button></div>
        ...
        <main className={`${!mobileDetail ? 'hidden lg:block' : 'block'} min-w-0 overflow-hidden rounded-2xl border border-[var(--border-light)] bg-[var(--bg-surface)] shadow-sm`}>
          {detail.isLoading ? <PanelLoadingSkeleton rows={6} label="Loading plan details" /> : current ? <>
            <header className="border-b border-[var(--border-light)] px-4 pt-4">
              <button type="button" onClick={() => setMobileDetail(false)} className="mb-3 inline-flex items-center gap-1 text-sm font-bold text-[var(--sgh-cyan-primary)] lg:hidden"><ChevronLeft size={16} />Plans</button>
              <div className="flex items-start justify-between gap-3">
                <h2 className="min-w-0 text-lg font-extrabold leading-6 text-[var(--text-primary)]">{current.name}</h2>
                {options.data.can_edit && <div className="flex shrink-0 items-center gap-1.5"><button type="button" onClick={() => setEditOpen(true)} className="inline-flex min-h-8 items-center gap-1 rounded-lg border border-[var(--sgh-cyan-primary)]/30 px-2 text-[11px] font-bold text-[var(--sgh-cyan-primary)] hover:bg-[var(--sgh-cyan-primary)]/10 transition-colors"><Pencil size={13} />Edit</button><button type="button" onClick={() => setDeleteOpen(true)} className="inline-flex min-h-8 items-center gap-1 rounded-lg border border-rose-500/25 px-2 text-[11px] font-bold text-rose-600 dark:text-rose-400 hover:bg-rose-500/10 transition-colors"><Trash2 size={13} />Delete</button></div>}
              </div>
              <div className="mt-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex shrink-0 items-center gap-1.5 rounded-lg bg-[var(--bg-sunken)]/55 px-2 py-1"><div className="grid h-7 w-7 place-items-center rounded-md bg-[var(--sgh-cyan-primary)]/10 text-[var(--sgh-cyan-primary)]"><Gauge size={13} /></div><div><span className="block text-[8px] font-bold uppercase leading-none text-[var(--text-faint)]">Plan progress</span><strong className="text-sm leading-4 text-[var(--text-primary)]">{current.progress.overall}%</strong></div></div>
              </div>
              ...
              <div className="mt-3 flex gap-1 overflow-x-auto">{tabs.map((value) => <button type="button" key={value} onClick={() => setTab(value)} className={`min-h-10 whitespace-nowrap border-b-2 px-2.5 text-xs font-bold transition-colors ${tab === value ? 'border-[var(--sgh-cyan-primary)] text-[var(--sgh-cyan-primary)]' : 'border-transparent text-[var(--text-muted)] hover:text-[var(--text-primary)]'}`}>{value}{value !== 'Overview' ? ` (${countFor(value)})` : ''}</button>)}</div>
            </header>
