...
const input = 'min-h-11 w-full rounded-xl border border-[var(--input-border)] bg-[var(--input-bg)] px-3 text-sm font-semibold text-[var(--input-text)] outline-none focus:border-[var(--sgh-cyan-primary)] focus:ring-2 focus:ring-[var(--sgh-cyan-primary)]/20 transition-all';
...
const statusStyles: Record<MilestoneStatus, string> = {
  ...
  'In Progress': 'border-cyan-200 bg-cyan-50 text-cyan-700 dark:border-cyan-500/30 dark:bg-cyan-500/10 dark:text-cyan-300',
...
function MilestoneFormModal({ detail, owners, milestone, onClose }: Props & { milestone: PlanMilestone | null; onClose: () => void }) {
  ...
  return <OverlayPortal><div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-3 backdrop-blur-sm"><form role="dialog" aria-modal="true" aria-labelledby="milestone-form-title" onSubmit={submit} className="w-full max-w-xl rounded-3xl border border-[var(--border-light)] bg-[var(--bg-surface)] shadow-2xl">
    ...
    <footer className="flex justify-end gap-2 border-t border-[var(--border-light)] p-4"><button type="button" onClick={onClose} className="min-h-11 rounded-xl border border-[var(--border-light)] px-4 font-bold text-[var(--text-secondary)] hover:bg-[var(--bg-sunken)] hover:text-[var(--text-primary)] transition-colors">Cancel</button><button type="submit" disabled={pending} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[var(--sgh-cyan-primary)] px-5 font-bold text-white shadow-sm hover:brightness-110 active:scale-[0.98] transition-all disabled:opacity-60">{pending ? <Loader2 size={16} className="animate-spin" /> : milestone ? <Pencil size={16} /> : <Plus size={16} />}{milestone ? 'Save changes' : 'Add step'}</button></footer>
...
export default function MilestonePanel({ detail, owners, canEdit }: Props) {
  ...
  return <section aria-label="Milestones and solution steps" className="space-y-3">
    <header className="flex flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="text-base font-extrabold text-[var(--text-primary)]">Milestones & solution steps</h3><p className="mt-0.5 text-xs text-[var(--text-muted)]">Add execution checkpoints, assign ownership and track whether each step is done.</p></div>{canEdit && <button type="button" onClick={() => setFormMilestone(null)} className="inline-flex min-h-9 items-center justify-center gap-1.5 rounded-lg border border-[var(--sgh-cyan-primary)]/30 bg-[var(--sgh-cyan-primary)]/10 px-3 text-xs font-bold text-[var(--sgh-cyan-primary)] transition-colors hover:bg-[var(--sgh-cyan-primary)]/20"><Plus size={14} />Add solution step</button>}</header>
    ...
    {detail.milestones.length ? <div className="grid gap-3 md:grid-cols-2">{detail.milestones.map((milestone) => <article key={milestone.id} className="rounded-2xl border border-[var(--border-light)] bg-[var(--bg-surface)] p-4">
      <div className="flex items-start justify-between gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[var(--sgh-cyan-primary)]/10 text-[var(--sgh-cyan-primary)]"><Flag size={17} /></span><span className={`rounded-full border px-2.5 py-1 text-[10px] font-extrabold ${statusStyles[milestone.status]}`}>{milestone.status}</span></div>
      ...
      {milestone.completion_date && <p className="mt-2 text-[10px] font-bold text-emerald-600 dark:text-emerald-400">Completed {formatDate(milestone.completion_date)}</p>}
      {canEdit && <div className="mt-4 flex flex-wrap gap-1.5 border-t border-[var(--border-light)] pt-3"><button type="button" disabled={update.isPending} onClick={() => toggleCompleted(milestone)} className={`inline-flex min-h-8 items-center gap-1 rounded-lg px-2.5 text-[11px] font-bold transition-colors ${milestone.status === 'Completed' ? 'border border-[var(--border-light)] text-[var(--text-secondary)] hover:bg-[var(--bg-sunken)]' : 'bg-emerald-600 text-white hover:bg-emerald-500'}`}>{milestone.status === 'Completed' ? <RotateCcw size={13} /> : <CheckCircle2 size={13} />}{milestone.status === 'Completed' ? 'Reopen' : 'Mark completed'}</button><button type="button" aria-label={`Edit ${milestone.name}`} onClick={() => setFormMilestone(milestone)} className="inline-flex min-h-8 items-center gap-1 rounded-lg border border-[var(--sgh-cyan-primary)]/30 px-2 text-[11px] font-bold text-[var(--sgh-cyan-primary)] hover:bg-[var(--sgh-cyan-primary)]/10 transition-colors"><Pencil size={13} />Edit</button><button type="button" aria-label={`Delete ${milestone.name}`} onClick={() => setDeleteMilestone(milestone)} className="inline-flex min-h-8 items-center gap-1 rounded-lg border border-rose-500/25 px-2 text-[11px] font-bold text-rose-600 dark:text-rose-400 hover:bg-rose-500/10 transition-colors"><Trash2 size={13} />Delete</button></div>}
