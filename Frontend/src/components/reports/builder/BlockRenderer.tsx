...
function MovementBridge({ data }: { data: ScoreMovementBridgeData }) {
  ...
  return <div className="space-y-4">
    <div className="grid grid-cols-3 gap-2"><div className="rounded-lg bg-slate-50 p-3 dark:bg-slate-800"><p className="text-[9px] font-bold uppercase text-slate-500 dark:text-slate-400">{data.comparison_period || 'Previous'}</p><p className="text-xl font-black">{display(data.previous_overall_score)}%</p></div><div className="rounded-lg bg-blue-50 p-3 text-center dark:bg-blue-950/30"><p className="text-[9px] font-bold uppercase text-[var(--sgh-cyan-primary,#00A3E0)]">Movement</p><p className={`text-xl font-black ${tone(data.total_score_point_change)}`}>{points(data.total_score_point_change)}</p></div><div className="rounded-lg bg-slate-50 p-3 text-right dark:bg-slate-800"><p className="text-[9px] font-bold uppercase text-slate-500 dark:text-slate-400">{data.current_period || 'Current'}</p><p className="text-xl font-black">{display(data.current_overall_score)}%</p></div></div>
...
function ManagementTable({ type, data, limit }: { type: string; data: ManagementAnalysisTableData; limit: number }) {
  ...
  return <div className="space-y-3"><div className="overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-700"><table className="w-full min-w-[680px] text-[10px]"><thead className="bg-slate-50 text-slate-500 dark:bg-slate-800 dark:text-slate-300"><tr>{columns.map(([key, label]) => <th key={key} className="px-3 py-2 text-left font-extrabold uppercase">{label}</th>)}</tr></thead><tbody>{rows.slice(0, limit).map((row, index) => <tr key={`${row.employee_id || row.key || row.code || index}`} className="border-t border-slate-100 align-top dark:border-slate-800">{columns.map(([key]) => <td key={key} className={`max-w-48 whitespace-normal px-3 py-2 font-medium leading-4 ${['lost_points', 'weighted_lost_points', 'trend'].includes(key) && Number(row[key]) < 0 ? 'text-rose-600 dark:text-rose-400' : 'text-slate-700 dark:text-slate-300'}`}>{managementCell(type, row, key)}</td>)}</tr>)}</tbody></table></div>
    ...
    {rows.length > limit ? <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400">Showing {limit} of {rows.length}</p> : null}
  </div>;
}
...
export default function BlockRenderer({ block, blockData, isEditing = false, onSelect }: {
  ...
  return <section onClick={onSelect} className={`h-full min-h-0 overflow-auto rounded-xl border bg-white p-4 text-left shadow-sm transition dark:bg-slate-900 ${isEditing ? 'cursor-pointer hover:border-[var(--sgh-cyan-primary,#00A3E0)]' : ''} ${blockData?.state === 'permission_denied' ? 'border-red-200 dark:border-red-900/50' : 'border-slate-200 dark:border-slate-700'}`}>
    <div className="mb-3 flex items-center justify-between gap-3"><h4 className="truncate text-sm font-extrabold text-slate-800 dark:text-slate-100">{block.config.title || titleCase(block.type)}</h4>{blockData ? <span className="rounded-md bg-slate-100 px-2 py-1 text-[9px] font-bold uppercase tracking-wide text-slate-500 dark:bg-slate-800 dark:text-slate-300">{blockData.source_periods.join(' vs ') || titleCase(blockData.state)}</span> : <InlineLoadingBadge />}</div>
    {!blockData ? <BlockLoadingState title={block.config.title || titleCase(block.type)} /> : noData ? <div className="flex min-h-24 flex-col items-center justify-center rounded-lg border border-dashed border-slate-200 bg-slate-50 p-4 text-center text-slate-500 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-400"><Database size={20} className="mb-2" /><p className="text-xs font-bold">{titleCase(blockData.state)}</p>{blockData.warnings.map((warning) => <p key={warning} className="mt-1 text-[10px]">{warning}</p>)}</div> : block.type === 'overall_score_movement_bridge' ? <MovementBridge data={payload as unknown as ScoreMovementBridgeData} /> : managementTable ? <ManagementTable type={block.type} data={payload as unknown as ManagementAnalysisTableData} limit={block.config.row_limit} /> : <>
      {metrics.length > 0 && <div className="grid grid-cols-2 gap-2 lg:grid-cols-4">{metrics.slice(0, 4).map((metric, index) => <div key={`${metric.label}-${index}`} className="rounded-lg bg-slate-50 p-3 dark:bg-slate-800"><p className="truncate text-[10px] font-bold uppercase text-slate-500 dark:text-slate-400">{display(metric.label)}</p><p className="mt-1 text-xl font-black text-slate-900 dark:text-white">{display(metric.display ?? metric.value)}</p>{Boolean(metric.change_display) && <p className={`mt-1 text-[10px] font-bold ${metric.movement === 'negative' ? 'text-red-600 dark:text-red-400' : 'text-emerald-600 dark:text-emerald-400'}`}>{display(metric.change_display)}</p>}</div>)}</div>}
      {(series.length > 0 || items.length > 0) && <div className="space-y-2">{[...series, ...items].slice(0, 8).map((item, index) => { const value = Number(item.value ?? item.impact ?? 0); const width = Math.max(3, Math.min(100, Math.abs(value))); return <div key={`${item.label}-${index}`} className="grid grid-cols-[minmax(90px,1fr)_2fr_64px] items-center gap-2 text-[10px]"><span className="truncate font-semibold text-slate-600 dark:text-slate-300">{display(item.label)}</span><span className="h-2 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800"><span className={`block h-full rounded-full ${value < 0 ? 'bg-rose-500' : 'bg-emerald-500'}`} style={{ width: `${width}%` }} /></span><b className={value < 0 ? 'text-rose-600 dark:text-rose-400' : 'text-emerald-600 dark:text-emerald-400'}>{display(item.display ?? value)}</b></div>; })}</div>}
      {rows.length > 0 && <div><div className="overflow-hidden rounded-lg border border-slate-100 dark:border-slate-800"><table className="w-full table-fixed text-[9px]"><thead className="bg-slate-50 text-slate-500 dark:bg-slate-800 dark:text-slate-300"><tr>{Object.keys(rows[0]).filter((key) => !['employee_id', 'is_below', 'validation', 'projection_assumptions'].includes(key)).slice(0, 5).map((key) => <th key={key} className="truncate px-2 py-2 text-left uppercase">{titleCase(key)}</th>)}</tr></thead><tbody>{rows.slice(0, Math.min(block.config.row_limit, 6)).map((row, rowIndex) => <tr key={rowIndex} className="border-t border-slate-100 dark:border-slate-800">{Object.keys(rows[0]).filter((key) => !['employee_id', 'is_below', 'validation', 'projection_assumptions'].includes(key)).slice(0, 5).map((key) => <td key={key} className="truncate px-2 py-2 font-medium text-slate-700 dark:text-slate-300">{display(row[key])}</td>)}</tr>)}</tbody></table></div>{rowSummary && Number(rowSummary.total || 0) > Number(rowSummary.shown || 0) && <p className="mt-2 text-[10px] font-bold text-[var(--text-muted)]">Showing {rowSummary.shown} of {rowSummary.total}</p>}</div>}
      {narrative && <div className="flex gap-3 rounded-lg bg-blue-50/70 p-3 text-xs leading-5 text-slate-700 dark:bg-blue-950/30 dark:text-slate-300"><FileText size={16} className="mt-0.5 shrink-0 text-[var(--sgh-cyan-primary,#00A3E0)]" /><p>{narrative}</p></div>}
      ...
      {blockData?.warnings.map((warning) => <div key={warning} className="mt-2 flex gap-2 text-[10px] font-semibold text-amber-700 dark:text-amber-300"><AlertTriangle size={12} />{warning}</div>)}
    </>}
  </section>;
}
