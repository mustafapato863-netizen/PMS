...
export default function Step3Builder() {
  ...
  return <div className="grid h-full min-h-[585px] grid-cols-[230px_minmax(0,1fr)_290px] overflow-hidden">
    ...
    <main className="min-w-0 overflow-auto p-6"><div className="mx-auto max-w-[1100px]"><div className="mb-3 flex items-center justify-between gap-3"><input value={activePage?.title || ''} onChange={(event) => activePage && store.updateSlide(activePage.id, { title: event.target.value })} className="min-w-0 flex-1 bg-transparent text-lg font-extrabold text-slate-800 outline-none dark:text-white" /><div className="flex items-center gap-2">{activePage && <button type="button" onClick={openAutomaticLibrary} disabled={!addableBlocks.length} className="flex items-center gap-1.5 rounded-lg bg-[var(--sgh-cyan-primary,#00A3E0)] px-3 py-2 text-xs font-extrabold text-white transition hover:bg-[var(--sgh-cyan-dark,#0084CE)] disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-400 dark:disabled:bg-slate-800" title={addableBlocks.length ? 'Add another block to this page' : 'This page has reached the supported block limit'}><Plus size={15} />{addableBlocks.length ? 'Add Block' : 'Page full'}</button>}<button type="button" onClick={openNewPageLibrary} className="flex items-center gap-1.5 rounded-lg border border-[var(--sgh-cyan-primary,#00A3E0)]/30 px-3 py-2 text-xs font-extrabold text-[var(--sgh-cyan-primary,#00A3E0)] hover:bg-[var(--sgh-cyan-primary,#00A3E0)]/10 dark:text-[var(--sgh-cyan-light,#38BDF8)]"><Plus size={15} />Block as page</button><label className="sr-only" htmlFor="report-page-layout">Page layout</label><select id="report-page-layout" value={activePage?.layout || ''} onChange={(event) => changeLayout(event.target.value)} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-800 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100">{pageLayoutOptions.map((item) => <option key={item.key} value={item.key}>{item.key.replace(/_/g, ' ')}</option>)}</select>{isFetching && <span className="text-xs text-slate-400">Refreshing data...</span>}</div></div>
      {!activePage ? <div className="grid aspect-video place-items-center rounded-2xl border-2 border-dashed border-slate-300 bg-[var(--bg-surface)] dark:border-slate-800 dark:bg-slate-900"><button onClick={addPage} className="rounded-xl bg-[var(--sgh-cyan-primary,#00A3E0)] px-5 py-3 font-bold text-white hover:bg-[var(--sgh-cyan-dark,#0084CE)]">Add first report page</button></div> : <div className="flex aspect-video flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white p-7 shadow-xl dark:border-slate-700 dark:bg-slate-950"><h2 className="border-b border-slate-100 pb-3 text-xl font-black text-slate-900 dark:border-slate-800 dark:text-white">{activePage.title}</h2><div className={`mt-4 grid min-h-0 flex-1 auto-rows-fr gap-4 ${activePage.blocks.length + emptySlots.length > 1 && !stackedLayout ? 'grid-cols-2' : 'grid-cols-1'}`}>{activePage.blocks.map((block, index) => <div key={block.id} className={`min-h-0 ${activePage.layout === 'team_review' && index === 2 ? 'col-span-2' : ''}`}><BlockRenderer block={block} blockData={pageData?.blocks[block.id]} isEditing onSelect={() => store.setActiveBlockId(block.id)} /></div>)}{emptySlots.map(([slot, allowed]) => <button key={slot} onClick={() => setLibrary({ slot, allowed })} className="grid min-h-0 place-items-center rounded-xl border-2 border-dashed border-slate-200 text-xs font-bold text-slate-400 hover:border-blue-400 hover:text-blue-600"><span className="flex items-center gap-2"><Plus size={16} /> Add {slot.replace(/_/g, ' ')} block</span></button>)}</div></div>}</div></main>
    <aside className="overflow-y-auto border-l border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900"><div className="mb-5 flex items-center gap-2"><Settings2 size={17} className="text-[var(--sgh-cyan-primary,#00A3E0)]" /><h3 className="font-extrabold text-slate-800 dark:text-white">Block settings</h3></div>{!activeBlock ? <p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-500 dark:bg-slate-800 dark:text-slate-400">Select a block on the page to configure it.</p> : <div className="space-y-5"><div><p className="text-sm font-extrabold text-slate-900 dark:text-white">{activeBlockMeta?.name || activeBlock.type}</p><p className="mt-1 text-xs leading-5 text-slate-500 dark:text-slate-400">{activeBlockMeta?.description}</p></div><label className="block text-xs font-bold text-slate-600 dark:text-slate-300">Custom title<input value={activeBlock.config.title || ''} onChange={(event) => store.updateBlock(activePage!.id, activeBlock.id, { config: { ...activeBlock.config, title: event.target.value || null } })} className="mt-1 w-full rounded-lg border border-slate-200 bg-transparent p-2.5 text-sm text-slate-900 outline-none focus:border-[var(--sgh-cyan-primary,#00A3E0)] dark:border-slate-700 dark:text-white" /></label><label className="block text-xs font-bold text-slate-600 dark:text-slate-300">Maximum rows<input type="number" min={1} max={50} value={activeBlock.config.row_limit} onChange={(event) => store.updateBlock(activePage!.id, activeBlock.id, { config: { ...activeBlock.config, row_limit: Number(event.target.value) } })} className="mt-1 w-full rounded-lg border border-slate-200 bg-transparent p-2.5 text-sm text-slate-900 dark:border-slate-700 dark:text-white" /></label>{activeBlock.type === 'management_commentary' && <label className="block text-xs font-bold text-slate-600 dark:text-slate-300">Management commentary<textarea value={store.commentary.entries[activeBlock.id] || ''} onChange={(event) => store.setCommentary(activeBlock.id, event.target.value)} rows={8} className="mt-1 w-full rounded-lg border border-slate-200 bg-transparent p-3 text-sm leading-6 text-slate-900 outline-none focus:border-[var(--sgh-cyan-primary,#00A3E0)] dark:border-slate-700 dark:text-white" /></label>}<button onClick={() => store.deleteBlock(activePage!.id, activeBlock.id)} className="flex w-full items-center justify-center gap-2 rounded-lg border border-red-200 py-2 text-sm font-bold text-red-600 hover:bg-red-50 dark:border-red-900/50 dark:text-red-400 dark:hover:bg-red-950/30"><Trash2 size={15} /> Remove block</button></div>}</aside>
